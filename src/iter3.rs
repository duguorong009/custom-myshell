        ...
        if command_args.len() > 0 {
            let child = match command_args[0] {
                "show" if command_args.len() > 1 => match  command_args[1] {
                    ...
                    _ => Err(Error::new(ErrorKind::InvalidInput, "Please enter valid command")),
                }, 
                "show" if command_args.len() == 1 => Err(Error::new(ErrorKind::InvalidInput, "Please enter valid command",)),
            ...
            match child {
                ...
                Err(e) => match e.kind() {
                    ErrorKind::InvalidInput => eprintln!("Sorry, show command only supports following options: files, process "),
